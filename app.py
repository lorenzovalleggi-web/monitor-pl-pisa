...
# ==========================================
# BINARIO LIBERO — Layout Ottimizzato
# ==========================================

st.set_page_config(
    page_title="Binario Libero", page_icon="🚧", layout="wide"
)
...
# Trova SOLO il prossimo treno utile per un dato PL
...
with st.sidebar:
    ...
    pl_selezionato = st.selectbox("Seleziona Passaggio a Livello:", list(PL_CONFIG.keys()))
    direzione = st.radio("Direzione:", ["San Giuliano → Pisa", "Pisa → San Giuliano"])

    cfg = PL_CONFIG[pl_selezionato]
    offset = cfg["offset_andata"] if direzione == "San Giuliano → Pisa" else cfg["offset_ritorno"]
    lista_orari = ORARI_ANDATA if direzione == "San Giuliano → Pisa" else ORARI_RITORNO
    ...
    for num, hhmm in lista_orari:
        ...
        if now_ita < apertura:
            st.write(f"🚆 **{num}** — Transito: `{transito.strftime('%H:%M')}`")
            st.caption(f"Chiusura: {chiusura.strftime('%H:%M')} ➜ Riapertura: {apertura.strftime('%H:%M')}")
            st.markdown("---")
            trovati += 1
