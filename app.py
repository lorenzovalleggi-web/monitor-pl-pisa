        ...
        for t in res.get('tabellone', []):
            ...
            if "PISA" in dest or "LIVORNO" in dest:
                ...
                if orario_prog:
                    ...
                    treni_attivi.append({"ora_p": h, "min_p": m, "ritardo": int(ritardo), "direzione": "PISA", "info": f"➔ **REG {t.get('numeroTreno')}** per {t.get('destinazione')}"})
    except: pass
        ...
        for t in res.get('tabellone', []):
            ...
            if "LUCCA" in dest or "PISTOIA" in dest or "FIRENZE" in dest:
                ...
                if orario_prog:
                    ...
                    ritardo = t.get('ritardo', 0)
                    if ritardo == "---" or ritardo is None: ritardo = 0
                    treni_attivi.append({"ora_p": h, "min_p": m, "ritardo": int(ritardo), "direzione": "LUCCA", "info": f"🡨 **REG {t.get('numeroTreno')}** per {t.get('destinazione')}"})
    except: pass
    return treni_attivi
...
if treni_futuri:
    ...
    nota_ritardo = f" (+{prox['ritardo']} min)" if prox['ritardo'] > 0 else " (In orario)"
    ...
    if ora_adesso.hour >= 22 or ora_adesso.hour < 5:
        prossimo_treno_testo = "Servizio giornaliero terminato. 🌅 Primo treno: REG 05:30 per Lucca / 05:51 per Pisa."
    else:
        prossimo_treno_testo = "Nessun transito imminente rilevato."

st.info(f"📋 **STATO LINEA LIVE:** {prossimo_treno_testo}")
st.caption("ℹ️ Treni merci non programmati potrebbero causare chiusure estemporanee.")

if ritardo_rilevato_linea:
    st.warning("⚠️ Rallentamento sulla tratta. Chiusure prolungate.")
...
st.markdown(" ")
st.write("📢 **Vuoi pubblicizzare la tua attività?**")
st.link_button("📩 Diventa Sponsor (Invia Email)", "mailto:info.railflow@gmail.com?subject=Sponsor%20RailFlow")
